...
public final class FormNodeFactory {
    ...
    public static FormNode makeNode(Element element) throws UnknownConfigFileTagNameException, UnknownTypeException,
        ...
        if ("blueprint".equals(tagName)) {
            List<AbstractEntry> sectionChildren = getEntries(element.getChildNodes());
    ...
    private static List<AbstractEntry> getEntries(NodeList nodeList) throws UnknownConfigFileTagNameException,
            UnknownTypeException, InvalidDefaultValueFormatException {
        List<AbstractEntry> output = new ArrayList<>(nodeList.getLength());
        for (int i = 0; i < nodeList.getLength(); i++) {
            Node childNode = nodeList.item(i);
            if (childNode.getNodeType() == Node.ELEMENT_NODE) {
                AbstractEntry newFormChild = EntryFactory.makeEntry((Element) childNode);
