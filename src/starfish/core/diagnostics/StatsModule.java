...
public class StatsModule extends CommandModule {
	...
	public void start() {
		...
		for (Material mat : getMaterialsList()) {
			pw.printf(",mass.%s,mom.%s.x,mom.%s.y,mom.%s.z,energy.%s", mat.name, mat.name, mat.name, mat.name,
					mat.name);
	...
	protected void saveStats() {
		...
		for (Material mat : getMaterialsList()) {
			pw.printf(",%.4g,%.4g,%.4g,%.4g,%.4g", mat.getMassSum(), mat.getMomentumSum()[0], mat.getMomentumSum()[1],
		...
		for (Boundary boundary : Starfish.getBoundaryList()) {
			for (Source source : boundary.getSourceList()) {
				...
				pw.printf(",%.4g", mass_gen * source.getMaterial().charge
						/ (source.getMaterial().mass * boundary.area() * stats_skip * Starfish.getDt())); // current den
