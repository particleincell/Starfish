...
public class InputParser implements Iterable {
	 ...
	 * @param file_name
	 */
	public InputParser(String file_name, String wd) {
		/* recursively load the entire file */
		root_element = Load(file_name, wd);
	}

	/**
	 * Recursively loads an input file, replaces {@code <load>} XML elements with
	 * the content from the specified file
	 * 
	 * @param file_name file to load
	 * @return root node of the loaded element
	 */
	protected final Node Load(String file_name, String wd) {
		...
		for (int i = 0; i < nodes.getLength(); i++) {
			...
			if (node instanceof Element) {
				if (node.getNodeName().equalsIgnoreCase("load")) {
					...
					Node new_node = document.importNode(Load(name,wd), true);
	...
	/**
	 * Searches source element for the specified key which can be given as an
	 * attribute or a node. If key exists as attribute, it returns the associated
	 * value If key exists as a node, the function returns a list of all node values
	 * with this key. Also returns the corresponding attribute if specified
	 * 
	 * @param key     key name
	 * @param element source element
	 * @return key value
	...
	public static ArrayList<AttValPair> getAttributeValueList(String key, String attribute, Element element)
		...
		/*
		 * get child nodes - using this instead of getElementsByTagName since that one
		 * is case sensitive
		 */
		...
		for (int i = 0; i < nodes.getLength(); i++) {
			...
			if (node.getNodeName().equalsIgnoreCase(key) && node.getNodeType() == Node.ELEMENT_NODE) {
	...
	private static String getValueInternal(String key, Element element) throws NoSuchElementException {
		...
		if (!list.isEmpty())
			...
			return null; // this shouldn't ever happen since getAttValList throws an exception
	}

	/**
	 * returns string value associated with the given key or the default if not
	 * found
	 * 
	...
	/**
	 * convenience method to parse "true" or "yes" as boolean, any other value
	 * results in false
	 * 
	 * @param key     key to search for
	 * @param element source element
	...
	/**
	 * convenience method to parse a boolean. Returns default value if key not
	 * found.
	 * 
	 * @param key           key to search for
	 * @param element       source element
	...
	/**
	 * convenience method to parse an integer. Returns a default value if key not
	 * found.
	 * 
	 * @param key           key to search for
		...
		try {
			...
			if (val.contains(","))
				Log.error(String.format(
						"Found a comma in '%s' in <%s>. Please use a dot to deliminate the fractional part.", val,
						key));
			return Double.parseDouble(getValueInternal(key, element));
		} catch (NoSuchElementException e) {
			return default_value;
	...
	/**
	 * returns values from a field such as &lt;node_name&gt;aa, bb,
	 * cc&lt;/node_name&gt; as string list strings separator is "," and
	 ...
	 * @param node_name
	 * @param element   *
	 * @return values from a field such as
	 *         {@code <node_name>[aa, bb], [cc, dd]</node_name>} as string list
	 */
	...
	/**
	 * @return values from a field such as {@code <key>[x1,y1], [x2,y2]</key>} as a
	 *         list of doubles
	 */
	...
	/**
	 * @return values from a field such as {@code <key>1*x, 2.0*y</key>} as a list
	 *         of <double,string> pairs
	 */
	...
	public static LinearList getLinearList(String key, String attr, Element element, double def) {
		...
		try {
			...
			for (AttValPair av_pair : in_list) {
				try {
					double t = 0;
					if (!av_pair.attribute.isEmpty())
						t = Double.parseDouble(av_pair.attribute);
					double val = Double.parseDouble(av_pair.value);
					list.insert(t, val);
				} catch (NumberFormatException e) {
					Log.warning("Could not parse attribute-key pair " + av_pair.attribute + ", " + av_pair.value
							+ " to two doubles");
				}
			}
		} catch (Exception e) {
