...
public class VolumeSource extends Source {
	 ...
	 * @return
	 */
	public Field2D getDn(Mesh mesh) {
		return dn.getField(mesh);
	}
	 ...
	 * @return
	 */
	public Field2D getTemp(Mesh mesh) {
		return source_mat.getT(mesh);
	}
	...
	public Particle sampleParticle() {
		...
		// negative temperature indicates we are in a boundary cell that has neighbors
		// set to -1
		if (temp_p < 50)
			temp_p = 50; // todo: for now added limit
