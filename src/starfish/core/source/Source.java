...
public abstract class Source {
	...
	public double getMassGenerated() {
		return mass_generated;
	}

	public double getMassGeneratedInst() {
		return mass_generated_inst;
	}

	public void clearMassGeneratedInst() {
		mass_generated += mass_generated_inst;
		mass_generated_inst = 0;
	} // called from stats
...
//	return mdot0*60*Math.exp(-0.013*t)/11.47 ;
	...
	/**
	 * default function to reset particle sample size, called prior to sampling
	 * source this can be overridden, but the default hook should be update()
	 */
	...
	final void sampleAll() {
		...
		if (source_mat instanceof KineticMaterial) {
			...
			// Log.error("Attempt to sample material of type " +
			// source_mat.getClass().getName());
		}
	}

	/**
	 * samples particles from a source, declared as final so that sources can't
	 * override (instead should override sampleParticle and hasParticle)
	 */
	final void sampleKinetic() {
		...
		while (hasParticles()) {
			...
			/*
			 * push particle slightly off the surface this is needed to avoid particles
			 * getting injected inside the surface if strong external retarding field
			 */
			for (int i = 0; i < 3; i++)
			...
			if (part != null && ks.addParticle(part)) {
				count++;
				mass_generated_inst += part.mpw * source_mat.mass;
			}
		}
