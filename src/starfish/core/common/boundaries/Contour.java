...
public class Contour 
{
	...
	{		
		...
		for (int m=0;m<Starfish.getMeshList().size();m++)
		{
			...
			if (Math.abs(v1-value)<tolerance) {found=true; break;}
			...
			do
			{
				...
				if (Math.abs(v2-value)<tolerance) {found=true; break;}
		...
		while (true)
		{
			...
			if (edge==0)	/*bottom*/
			{
				...
				{
					//TODO: 10/10/2019: think about how to support contouring on multi-meshes
			...
			{
				edge=3;
				if (i==field.getNi()-2 && mesh.boundaryType(Face.RIGHT,j)==DomainBoundaryType.MESH)
				{
			...
			{
				edge=0;
				if (j==field.getNj()-2 && mesh.boundaryType(Face.TOP,i)==DomainBoundaryType.MESH)
				{
