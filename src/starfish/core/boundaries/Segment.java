...
/**
 * Segment is the basic building block of a spline. A segment connects two
 * points either linearly or as a smooth cubic bezier curve
 */
public abstract class Segment {
	 ...
	 * @param id
	 */
	public void setParentInfo(Boundary boundary, int id) {
	...
	/**
	 * Computes segment area. For XY domain, this is the same as length (depth=1 is
	 * assumed). For RZ/ZR domains, this is the swept area (currently correctly
	 * defined only for linear segments)
	 * 
	 ...
	 * 
	 * @param s2
	 * @return t parametric intersection along parent segment in t[0], parametric
	 *         intersection along segment s2 in t[1]
	 */
	...
	boolean colinearWith(Segment segment) {
		...
		if (Math.abs(acos - 1.0) < 1e-6)
			return true;
		return false;
	}

	/**
	 * @param b1 * @return a list of segments either intersecting or fully located
	 *           in a box
	 * @param b2
	 */
	...
	public static boolean inBox(double p[], double b1[], double b2[]) {
		if (p[0] >= b1[0] && p[0] <= b2[0] && p[1] >= b1[1] && p[1] <= b2[1])
			return true;
		return false;
	}
	...
	//returns parametric t of the closest position on the segment to a point x
	//by performing num_steps binary-search steps
	public double nearestPosition(double[] x, int num_steps) {
		double a=0,c=1.0;
		double t=0.5;
