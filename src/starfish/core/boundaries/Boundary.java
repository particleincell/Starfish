...
public class Boundary extends Spline {
	...
	protected double temp; // last evaluated temperature, saved so we don't need to re-evaluate for every
							// particles
	...
	public Boundary(Element element) {
		...
		else if (type_name.equalsIgnoreCase("SOLID"))
			type = BoundaryType.DIRICHLET;
		else if (type_name.equalsIgnoreCase("INSULATOR"))
			type = BoundaryType.INSULATOR;
		// legacy support, symmetry should be set on meshes
		else if (type_name.equalsIgnoreCase("SYMMETRY"))
			type = BoundaryType.VIRTUAL;
		else if (type_name.equalsIgnoreCase("VIRTUAL"))
			type = BoundaryType.VIRTUAL;
		else if (type_name.equalsIgnoreCase("SINK"))
			type = BoundaryType.SINK;
		else {
			Starfish.Log.error("Unknown boundary type " + type_name);
			type = null;
		}
	...
	static public enum BoundaryType {
		DIRICHLET(0), OPEN(1), SINK(2), VIRTUAL(3), INSULATOR(4);

		protected int val;

		BoundaryType(int val) {
			this.val = val;
		}
	 ...
	 * 
	 * @return true if value changed. This then causes mesh information to be
	 *         updated as well.
	 */
	@Override
	 ...
	 * 
	 * @param material
	 * @return thermal velocity of the given material using surface temperature at
	 *         current time
	 */
	 ...
	 * @return
	 */
	public Material getMaterial(double t) {
		return material;
	} /* TODO: for now this assumes uniform mat */
